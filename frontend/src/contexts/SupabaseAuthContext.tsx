...
import { supabaseAuthService, LoginRequest } from '@/services/supabaseAuthService'
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabaseAuthService.onAuthStateChange((user) => {
      console.log('🔄 Cambio en estado de autenticación:', user ? 'Usuario logueado' : 'Usuario deslogueado')
      setUser(user)
      setIsLoading(false)
    })
  ...
  const signUp = async (credentials: LoginRequest): Promise<boolean> => {
    try {
      setIsLoading(true)
      const response = await supabaseAuthService.signUp(credentials)
