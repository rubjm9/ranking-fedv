...
const SURFACES = [
  {
    ...
    badgeColor: 'bg-brand-subtle text-brand-strong',
  },
  {
    name: 'Césped',
    categories: ['Mixto', 'Open', 'Women'],
  ...
  return (
    <section className="mb-8 rounded-[2rem] border border-line bg-gradient-to-b from-white to-secondary-50/70 px-4 py-10 shadow-sm sm:px-6 lg:px-8">
      <div className="mb-12 text-center">
        ...
        <p className="mx-auto max-w-3xl text-lg leading-8 text-content-muted">
      ...
      <div className="relative">
        ...
        <div className="space-y-8">
          <RankingOnboardingStep step={1} title="Participación en torneos" icon={<Trophy className="h-5 w-5" />}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {SURFACES.map((surface) => (
                <div key={surface.name} className="rounded-xl border border-line bg-surface/80 p-4">
                  <h4 className="mb-3 text-lg font-semibold text-content">{surface.name}</h4>
                  <div className="space-y-2">
                    {surface.categories.map((category) => (
                      <div key={category} className="flex items-center justify-between rounded-lg bg-surface-muted p-3">
            ...
            <div className="mt-5 rounded-xl border border-line bg-surface-muted p-4 text-sm text-content-muted">
              <strong className="text-content">6 modalidades independientes:</strong> playa mixto,
          ...
          <RankingOnboardingStep step={2} title="Puntos por posición" icon={<Calculator className="h-5 w-5" />}>
            <p className="mb-4 text-lg text-content-muted">
            ...
            <div className="mb-6 flex justify-center">
              <button
                ...
                className="inline-flex items-center gap-1.5 min-h-[44px] touch-manipulation rounded-full border border-brand-strong/30 bg-surface px-4 py-2 text-sm font-medium text-link transition-colors hover:border-primary-300 hover:text-brand-strong focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
            ...
            <div className="mb-5 grid grid-cols-1 gap-x-8 gap-y-4 sm:grid-cols-2">
              {COMBINATIONS.map((combination) => (
                <div
                  ...
                  className="border-b border-line/80 pb-4 last:border-b-0 sm:border-b sm:pb-4"
                >
                  <p className="text-base font-semibold text-content">{combination.title}</p>
                  <p className="mt-1 text-sm leading-6 text-content-muted">{combination.description}</p>
                </div>
              ))}
            </div>
